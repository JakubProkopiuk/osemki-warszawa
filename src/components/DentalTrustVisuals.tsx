...
export function DentalClinicVisual({ compact = false }: DentalClinicVisualProps) {
  return (
    <div className={`relative overflow-hidden rounded-[1.75rem] border border-slate-200 bg-white p-3 shadow-xl shadow-emerald-950/5 ${compact ? 'min-h-44' : 'min-h-64'}`}>
      <div className="absolute inset-x-4 top-4 h-px bg-gradient-to-r from-transparent via-emerald-100 to-transparent" />
      ...
      <div className={`relative overflow-hidden rounded-[1.35rem] bg-emerald-950 ${compact ? 'aspect-[16/10]' : 'aspect-[4/3]'}`}>
        <Image
          ...
          priority={!compact}
        />
        <div className="absolute inset-0 bg-gradient-to-t from-emerald-950/58 via-emerald-950/5 to-white/5" />
        <div className="absolute bottom-3 left-3 right-3 rounded-2xl border border-white/25 bg-white/92 p-3 shadow-lg backdrop-blur">
      ...
      <div className="relative mt-4 grid gap-2 sm:grid-cols-3">
        {[
          ...
          <div key={label} className="rounded-2xl border border-slate-100 bg-slate-50 px-3 py-2 text-center text-xs font-black text-emerald-800 shadow-sm">
            {label}
          </div>
        ))}
      </div>
...
export function TriageStepVisual({ stepId, compact = false }: TriageStepVisualProps) {
  ...
  return (
    <div className={`relative overflow-hidden rounded-[1.35rem] border border-slate-200 bg-slate-50 shadow-sm ${compact ? 'h-24 w-32' : 'h-36 w-44'}`}>
      <Image
        src={visual.src}
        alt={visual.alt}
        fill

...
// Stylistyka Premium (Glassmorphism)
...
export default function LocationClient({ locationData }: { locationData: any }) {
  ...
  const landmark = locationData.punkt_orientacyjny || "lokalnych punktów komunikacyjnych";

  // --- POPRAWKA FORMULARZA (HANDLERS) ---

  // Funkcja formatująca numer telefonu (maska: XXX-XXX-XXX)
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const res = await fetch('https://hook.eu1.make.com/k73x9s65dxykfhry5uyodhl6bg2kvkx7', {
        ...
        body: JSON.stringify({ ...formData, phone: rawPhone, slug: locationData.slug, locationName }),
      });
      ...
      <main className="relative z-10 w-full max-w-[1140px] mt-12 md:mt-24 text-left">
        <div className="grid lg:grid-cols-[1.1fr,0.9fr] gap-16 md:gap-24 items-start mb-32">
          ...
          >
            <div className="space-y-6">
              ...
              <p className="text-xl md:text-2xl text-slate-400 font-medium leading-relaxed max-w-lg">
                Profesjonalne usuwanie ósemek. Lokalizacja: <strong>{locationName}</strong>. Zapraszamy do gabinetu przy <strong>ul. {locationData.klinika}</strong>, blisko <strong>{landmark}</strong>. <br />
            ...
            <div className="flex flex-col md:flex-row items-center gap-8 p-8 rounded-[40px] bg-white border border-slate-100 shadow-xl relative overflow-hidden group">
              <div className="absolute top-0 right-0 w-32 h-32 bg-blue-50 rounded-bl-full opacity-50 transition-all group-hover:scale-110" />
              ...
              <div className="relative z-10 text-center md:text-left">
                ...
                <p className="text-slate-400 font-medium leading-relaxed italic text-sm">
                  Twój lekarz prowadzący w lokalizacji {doctor.locName}.
                </p>
              </div>
            </div>
          ...
          <motion.div 
            ...
            className={`${cardStyle} p-10 md:p-14 relative overflow-hidden`}
          >
            {status === 'success' ? (
              <div className="text-center py-20">
                ...
                <p className="text-slate-500 mt-4 leading-relaxed">Nasz chirurg dyżurny oddzwoni w ciągu kilku minut.</p>
              </div>
            ) : (
              <form onSubmit={handleSubmit} className="space-y-10">
                ...
                <button type="submit" disabled={status === 'loading'} className="relative overflow-hidden w-full bg-[#1A1C1E] text-white font-black text-lg py-6 rounded-3xl transition-all duration-300 hover:bg-blue-600 shadow-2xl hover:shadow-blue-200 active:scale-95 group uppercase tracking-widest">
        ...
        {/* --- POPRAWKA MAPY (Wymuszono HTTPS i poprawny URL) --- */}
        <div className="mb-32 text-left">
          <div className="flex items-center gap-4 mb-16">
            <h2 className="text-4xl font-black tracking-tight italic uppercase italic">Mapa Dojazdu</h2>
            <div className="h-px flex-grow bg-slate-100 rounded-full" />
          </div>
          <div className="w-full h-[500px] rounded-[56px] overflow-hidden border-8 border-white shadow-2xl grayscale hover:grayscale-0 transition-all duration-700 bg-slate-50 relative">
            <iframe
              width="100%"
              height="100%"
              style={{ border: 0 }}
              ...
              src={`https://maps.google.com/maps?q=${encodeURIComponent('Ochota na Uśmiech Warszawa ' + locationData.klinika)}&t=&z=16&ie=UTF8&iwloc=&output=embed`}
            ></iframe>
          </div>
        </div>
      </main>
